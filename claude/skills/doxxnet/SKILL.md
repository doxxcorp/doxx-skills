...
- doxx.net native iOS/Android app → `create_native_tunnel` (requires `device_hash` from `device_list_unified`). No WireGuard config or QR code needed: app handles connection. The tunnel record `type` field will still show `"wireguard"` (the protocol); the "native" distinction is at the client level, not the record.

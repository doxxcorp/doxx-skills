...
Use the `Read` tool on `~/.config/doxxnet/token` to check if a token exists. If it does, validate: `curl -s -X POST https://config.doxx.net/v1/ -d "auth=1&token=${DOXXNET_TOKEN:-$(cat ~/.config/doxxnet/token)}"` (substitute the actual token value)

If no token or validation fails, ask: "Do you have a doxx.net auth token?"

**If yes:** validate with `curl -s -X POST https://config.doxx.net/v1/ -d "auth=1&token=${DOXXNET_TOKEN:-$(cat ~/.config/doxxnet/token)}"`. On success, save it with the `Write` tool to `~/.config/doxxnet/token`.
